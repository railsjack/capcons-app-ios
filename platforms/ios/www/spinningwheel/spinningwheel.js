...
var SpinningWheel = {
	...
	create: function () {
		...
		for (l = 0; l < this.slotData.length; l += 1) {
			...
			for (i in this.slotData[l].values) {
				if(typeof this.slotData[l].values[i] ==='object'){
					out += '<li>' + this.slotData[l].values[i].value + '</li>';
				}else{
	...
	scrollToValue: function (slot, value) {
		...
		for (i in this.slotData[slot].values) {
			if(typeof this.slotData[slot].values[i] ==='object'){
				if(this.slotData[slot].values[i].key == value){
