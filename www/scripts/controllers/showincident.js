...
angular.module('CCS-Safety')
  .controller('ShowincidentCtrl', ['$rootScope', '$scope', '$location', '$routeParams', 'Api', 'Sharedata',
    function ($rootScope, $scope, $location, $routeParams, Api, Sharedata) {
        ...
        Api.get(settings.url + 'projects/' + projectId + '/incidents/' + incidentId + '/view_incident.json')
          .then(function (data) {
              if (data.error) {
                  ...
                  $scope.incident = data;
                  var records = _.map(data.incidents_files, function (incidents_file) {
                      return settings.url + incidents_file.file.url;
                  });
        ...
        $("#emailForm").validate({
            ...
            submitHandler: function () {
                ...
                var promise = Api.post(settings.url + 'incidents/' + $routeParams.id + '/send_email_by_incident.json', { email: $scope.user1.email });
